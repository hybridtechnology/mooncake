...
public abstract class BaseThemeFragment extends Fragment {
    ...
    public void onViewCreated(View view, @Nullable Bundle savedInstanceState) {
        ...
        if (settingPrefManager.getSetting().isAutoTimezone()) {
            ...
            Timber.e("TimeZone   " + tz.getDisplayName(false, TimeZone.SHORT) + " Timezon id :: " + tz.getID());
    ...
    TimeTickChangedListener timeTickChangedListener = () -> {
        ...
        //Toast.makeText(getActivity(),"Base Theme Time Tick"+settingPrefManager.getSetting().getTimeZoneGMT() +" : "+tz.getDisplayName(false, TimeZone.SHORT),Toast.LENGTH_SHORT).show();
        hours = calendar.get(Calendar.HOUR_OF_DAY);
        minutes = calendar.get(Calendar.MINUTE);
        updateTime(hours, minutes);
        int currentDayNumber = calendar.get(Calendar.DAY_OF_MONTH);
        if (dayNumber != currentDayNumber) updateDay();
    };

    public void timezoneChange() {
        Toast.makeText(getActivity(),"timezoneChange "+settingPrefManager.getSetting().getTimeZoneGMT(),Toast.LENGTH_SHORT).show();
        calendar = Calendar.getInstance();
        if (settingPrefManager.getSetting().isAutoTimezone()) {
    ...
    public void displayLoadingDialog() {
        new Handler().post(() -> {
            hideKeyboard();
            if (dialogLoading == null || getActivity().isFinishing()) return;
            dialogLoading.show();
        });
    }
    ...
    public void hideKeyboard() {
        ...
        if (imm.isActive()) imm.hideSoftInputFromWindow(getView().getWindowToken(), 0);
    }
